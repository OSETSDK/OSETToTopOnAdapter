//
//  OSETCustomBaseAdapter.m
//  OSETATADemo
//
//  Created by Shens on 25/11/2025.
//

#import "OSETCustomBaseAdapter.h"

@implementation OSETCustomBaseAdapter

#pragma mark - adapter init class name define
- (Class)initializeClassName {
    return [OSETCustomInitAdapter class];
}
...
+ (NSMutableDictionary *)getLossInfoResult:(ATBidWinLossResult *)winLossResult {
    ...
    NSMutableDictionary *infoDic = [NSMutableDictionary dictionary];
    [infoDic AT_setDictValue:winLossResult.winPrice key:@"_WIN_PRICE_"];
    //竞败原因 (1：竞争力不足 101：未参与竞价 10001：其他)
    [infoDic AT_setDictValue:errorCode key:@"_LOSS_REASON_"];
    //竞胜方渠道ID (1：美数其他非bidding广告位 2：第三方ADN 3：自售广告主 4：美数其他bidding广告位)
    [infoDic AT_setDictValue:winADN key:@"_ADN_ID_"];
    return infoDic;
}
...
+ (NSMutableDictionary *)getWinInfoResult:(ATBidWinLossResult *)winLossResult {
    NSMutableDictionary *infoDic = [NSMutableDictionary dictionary];
    //通常需要回传二价
    [infoDic AT_setDictValue:winLossResult.secondPrice key:@"_LOSS_PRICE_"];
    return infoDic;
}
